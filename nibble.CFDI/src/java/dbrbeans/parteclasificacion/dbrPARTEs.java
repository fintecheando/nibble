...
public class dbrPARTEs {
	...
	public boolean findByPar(int iidcategoria, int iidgrupo, int iidlinea,int iidmarca, String prefijo, String numero, String sufijo, String medida, boolean bitdadodebaja) throws Exception, java.sql.SQLException {
        ...
        public boolean findByDescripcion(String descripcion) throws Exception, java.sql.SQLException {
		StringBuffer query = new StringBuffer();
		dbrPARTE PARTE;

		
	
		query.append("SELECT * FROM parte WHERE (lower(VCHDESCRIPCIONES) LIKE ? OR lower(FICHATECNICA) LIKE ? OR lower(VCHOBSERVACIONES) LIKE ?)");
		...
		while (rSet.next()) {
			...
			PARTE.setIidmarca(rSet.getInt("iidmarca"));
			PARTE.setFichatecnica(rSet.getString("fichatecnica"));
			PARTE.setVchcategoria(rSet.getString("iidcategoria"));
			PARTE.setVchgrupo (rSet.getString("iidgrupo"));
			PARTE.setVchlinea(rSet.getString("iidlinea"));
			PARTE.setVchmarca(rSet.getString("iidmarca"));
			result.addElement(PARTE);
		}
		rSet.close();
		stmt.close();
