...
public class Devoluciones extends HttpServlet {
	...
	private void cancelaFactura(
		...
		//dbNOTADECREDITO bonificacion = new dbNOTADECREDITO();
		...
		//bonificacion.setConnection(con);
		...
		if (ncs.getResult().size() > 0) {
			throw new Exception(
				"Ya se han registrado notas de credito para la factura numero "
					+ bean92.getIidfactura()
		...
		for (int i = 0; i < bean92.getVPartidasSize(); i++) {
			...
			ncd.setDecimportedevol(
				bean92.getIcantidadproducto(i) * bean92.getDecprecioventa(i));
			ncd.create();
			...
			inventario.load();
			inventario.setIexistencia(
				inventario.getIexistencia() + bean92.getIcantidadproducto(i));
			inventario.store();
			...
			movimiento.setDecprecioventa(
				(bean92.getDecprecioventa(i) * (100 - fac.getDecdescglobal()))
					/ 100);
			...
			consumo.setMm(mm);
			consumo.setYy(yy);
			consumo.setIidproveedor(bean92.getIidproveedor(i));
			consumo.setLiidparte(bean92.getLiidparte(i));
			if (consumo.load()) {
				consumo.setCantidad(
					consumo.getCantidad() - bean92.getIcantidadproducto(i));
				consumo.store();
			} else {
				consumo.setCantidad(-bean92.getIcantidadproducto(i));
				consumo.create();
			}
		...
		float total =
			fac.getDecsubtotal() - fac.getDecmontodesc() + fac.getDecivatotal();
		...
		if (total - saldo > 0) {
			...
			dbai.setDechaber(0);
			dbai.setVchconcepto(
				"Aviso generado por Nota de Credito " + notadecredito);
			dbai.create();
			/*
		...
		bean.setFecha(hoy);
		bean.setOrigen("Cancelacion. Fac. " + fac.getIidfactura());
	...
	private void devolver(
		...
		for (int i = 0; i < bean92.getVPartidasSize(); i++) {
			if ((request.getParameter("cantidad" + i) != null)
				&& (request.getParameter("cantidad" + i).length() > 0)) {
				cantidad[i] =
					new Integer(request.getParameter("cantidad" + i))
						.intValue();
			}
		...
		for (int i = 0; i < bean92.getVPartidasSize(); i++) {
			if (cantidad[i] > 0) {
				devueltas =
					de.getDevueltasByFacturaParte(
						bean92.getIidfactura(),
						bean92.getLiidparte(i),
						bean92.getIidproveedor(i));

				if ((bean92.getIcantidadproducto(i) - devueltas)
					< cantidad[i]) {
		...
		// Procede en caso de poderse realizar la operaci�n
			...
			for (int i = 0; i < bean92.getVPartidasSize(); i++)
				if (cantidad[i] > 0) {
					...
					ncd.setDecivaclasific(bean92.getDecivaclasific(i));
					ncd.setDecimportedevol(
						cantidad[i] * bean92.getDecprecioventa(i));
					ncd.create();
					...
					inventario.load();
					inventario.setIexistencia(
						inventario.getIexistencia() + cantidad[i]);
					inventario.store();
					...
					movimiento.setDecporciva(bean92.getDecivaclasific(i));
					movimiento.setDecprecioventa(
						(bean92.getDecprecioventa(i)
							* (100 - fac.getDecdescglobal()))
							/ 100);
					movimiento.setIidreferencia(notadecredito);
					movimiento.setIidusuario(iidusuario);
					...
					if (consumo.load()) {
						consumo.setCantidad(
							consumo.getCantidad() - cantidad[i]);
						consumo.store();
					} else {
						consumo.setCantidad(-cantidad[i]);
						consumo.create();
					}
			...
			if (bean92.isMostradorSinIVA()) {
				...
				montodescuento =
					Calcula.redondeo((subtotal * fac.getDecdescglobal()) / 100);
				montoiva =
					Calcula.redondeo(
						(subtotal - montodescuento) * (bean92.getIva() / 100));
			...
			float saldoBonificacion = cxcs.findSaldoContraDocto(Idcontradocto,TIPOMOV_BONI);			
			...
			//	nc.setInumero;
			nc.setTisfechanotacredit(hoy);
			nc.setVchconcepto("Nota de Credito Generada Por Una Devolucion de la Factura" + fac.getIidfactura());
			
			nc.setDecimporte(subtotal+saldoBonificacionSinIVA);
			...
			cliente.load();	
			//cliente.setDeccreditutilizado(cliente.getDeccreditutilizado() + total);
			float saldoAbono = cxcs.findAbonosDocto(fac.getIidfactura(), TIPOMOV_BONI);
			float saldoCargo = cxcs.findCargosDocto(fac.getIidfactura(), TIPOMOV_FAC);
			float saldoTotal = saldoCargo-saldoAbono;
			int TIPOMOV_PAGO =4;
			float PagosFactura = cxcs.findPagosFactura(fac.getIidfactura(), TIPOMOV_PAGO);
			
			int FacturaSaldada = cxcs.findFacturaSaldada(fac.getIidfactura(), TIPOMOV_FAC);			
			
			if (FacturaSaldada !=1){			
				cliente.setDeccreditutilizado(cliente.getDeccreditutilizado() - (total-PagosFactura + saldoBonificacion));
				//bean92.setConceptoBonificacion("Menos Concepto de Bonificacion ");
			//	bean92.setBonificacion(saldoBonificacion);
				
			}
			else{
				cliente.setDeccreditutilizado(cliente.getDeccreditutilizado() + 0);
			}			
			cliente.store();
			...
			if (FacturaSaldada !=1){			
						...
						bean.setConceptoBonificacion("Menos Concepto de Bonificacion ");
						bean.setBonificacion(saldoBonificacion);
				
						}
						else{
							cliente.setDeccreditutilizado(cliente.getDeccreditutilizado() + 0);
							bean.setConceptoBonificacion("");
	...
	public void init() {
		idModulo =
			(getServletConfig().getInitParameter("idModulo") != null)
				? new Integer(getServletConfig().getInitParameter("idModulo"))
					.intValue()
				: (-1);
		ID_CONFIG =
			new Integer(getServletConfig().getInitParameter("ID_CONFIG"))
				.intValue();
		ID_CONFIGPVA =
			new Integer(getServletConfig().getInitParameter("ID_CONFIGPVA"))
				.intValue();
		ID_TIPOMOVIMIENTO =
			new Short(getServletConfig().getInitParameter("ID_TIPOMOVIMIENTO"))
				.shortValue();
		TIPOMOV_FAC =
			new Short(getServletConfig().getInitParameter("TIPOMOV_FAC"))
				.shortValue();
		TIPOMOV_NCR =
			new Short(getServletConfig().getInitParameter("TIPOMOV_NCR"))
				.shortValue();
		IDCLIENTESINIVA =
			new Integer(getServletConfig().getInitParameter("IDCLIENTESINIVA"))
				.intValue();
		IDCLIENTECONIVA =
			new Integer(getServletConfig().getInitParameter("IDCLIENTECONIVA"))
				.intValue();
		ID_MOV_DEVOLUCION =
			new Integer(
				getServletConfig().getInitParameter("ID_MOV_DEVOLUCION"))
				.intValue();
		ID_MOV_CANCELACION =
			new Integer(
				getServletConfig().getInitParameter("ID_MOV_CANCELACION"))
				.intValue();
	}
	...
	private void loadFactura(
		...
		if ((request.getParameter("iidfactura") != null)
			&& (request.getParameter("iidfactura").length() > 0)) {
			iidfactura =
				new Integer(request.getParameter("iidfactura")).intValue();
		}
		...
		int Idcontradocto = new Integer(request.getParameter("iidfactura")).intValue();
		int TIPOMOV_BONI = 6;						
		float saldoBonificacion = cxcs.findSaldoContraDocto(Idcontradocto,TIPOMOV_BONI);
		
		bean92.setConceptoBonificacion("Menos Concepto de Bonificacion ");
		bean92.setBonificacion(saldoBonificacion);
		bean92.setReferenciaBonificacion(Idcontradocto);		
		bean92.setFechaBonificacion((factura.getTisfechafactura()));
	...
	public void performTask(
		...
		try {
			...
			try {
				...
				dispatcher =
					getServletContext().getRequestDispatcher(strAddress);
